...
class GetRepairRequests(APIView):
    permission_classes = [AllowAny]

    def get(self, request, client_name):
        if not AuthHelper.authenticate_vendor_request(request):
            return Response(status=status.HTTP_401_UNAUTHORIZED)
        return RepairHandler.handle_get_repairs_for_vendor(request.data)


class GetRepairRequestDetails(APIView):
    permission_classes = [AllowAny]

    def get(self, request, client_name):
        if not AuthHelper.authenticate_vendor_request(request):
            return Response(status=status.HTTP_401_UNAUTHORIZED)
        return RepairHandler.handle_get_repair_details_for_vendor(request.data)
...
class UpdateRepairStatus(APIView):
    ...
    def post(self, request, client_name):
        ...
        try:
            ...
            repair_id = status_data.get("repair_id")
            ...
            repair_obj, res = RepairHelper.get_repair_request_by_id(repair_id, db_name)
            if res.status_code != status.HTTP_302_FOUND:
                return res

            # Update status
            status_update_response = RepairHandler.handle_update_repair_status(
            ...
            if len(request_files) > 0 :
                file_upload_response = RepairHandler.handle_add_supporting_files(
                    repair_obj, file_specs, request_files, user
                )
...
class UploadRepairFiles(APIView):
    permission_classes = [AllowAny]
    ...
    def post(self, request, client_name):
        ...
        try:
            ...
            return RepairHandler.handle_add_supporting_files(
                repair_obj, file_specs, request_files, user
            )
